...
savefig(fig, 'example_res_conv_AFD_square/searching_dictionary.jpg')
# generate evaluator
afdcal.genEva()

print("Time of generating evaluators: {:n} s".format(afdcal.time_genEva))
fig, _ = afdcal.plot_base_random()
savefig(fig, 'example_res_conv_AFD_square/evaluator.jpg')
# Initilize decomposition
afdcal.init_decomp()

print("Time of decomposition at level={:n}: {:n} s".format(0,afdcal.run_time[0]))
fig, _ = afdcal.plot_decomp(0)
savefig(fig, 'example_res_conv_AFD_square/decomp_comp_level_{:n}.jpg'.format(0))
fig, _ = afdcal.plot_basis_comp(0)
savefig(fig, 'example_res_conv_AFD_square/basis_comp_level_{:n}.jpg'.format(0))
fig, _ = afdcal.plot_remainder(0)
savefig(fig, 'example_res_conv_AFD_square/remainder_level_{:n}.jpg'.format(0))
...
for level in range(10):
    ...
    savefig(fig, 'example_res_conv_AFD_square/decomp_comp_level_{:n}.jpg'.format(afdcal.level))
    fig, _ = afdcal.plot_basis_comp(afdcal.level)
    savefig(fig, 'example_res_conv_AFD_square/basis_comp_level_{:n}.jpg'.format(afdcal.level))
    fig, _ = afdcal.plot_re_sig(afdcal.level)
    savefig(fig, 'example_res_conv_AFD_square/reconstructed_signal_level_{:n}.jpg'.format(afdcal.level))
    fig, _ = afdcal.plot_energy_rate(afdcal.level)
    savefig(fig, 'example_res_conv_AFD_square/energy_convergence_rate_level_{:n}.jpg'.format(afdcal.level))
    fig, _ = afdcal.plot_searchRes(afdcal.level)
    savefig(fig, 'example_res_conv_AFD_square/searching_result_level_{:n}.jpg'.format(afdcal.level))
    fig, _ = afdcal.plot_remainder(afdcal.level)
    savefig(fig, 'example_res_conv_AFD_square/remainder_level_{:n}.jpg'.format(afdcal.level))
    fig, _ = afdcal.plot_an(afdcal.level)
    savefig(fig, 'example_res_conv_AFD_square/an_level_{:n}.jpg'.format(afdcal.level))