...
for i_ch in range(ch_num):
    ...
    savefig(fig, 'example_res_multichannel_fast_AFD/searching_dictionary_ch_{:n}.jpg'.format(i_ch))
# generate evaluator
afdcal.genEva()
...
for i_ch in range(ch_num):
    ...
    savefig(fig, 'example_res_multichannel_fast_AFD/evaluator_ch_{:n}.jpg'.format(i_ch))
...
for i_ch in range(ch_num):
    ...
    savefig(fig, 'example_res_multichannel_fast_AFD/decomp_comp_level_{:n}_ch_{:n}.jpg'.format(0, i_ch))
    fig, _ = afdcal.plot_basis_comp(0, i_ch = i_ch)
    savefig(fig, 'example_res_multichannel_fast_AFD/basis_comp_level_{:n}_ch_{:n}.jpg'.format(0, i_ch))
    fig, _ = afdcal.plot_remainder(0, i_ch = i_ch)
    savefig(fig, 'example_res_multichannel_fast_AFD/remainder_level_{:n}_ch_{:n}.jpg'.format(0, i_ch))
...
for level in range(4):
    ...
    for i_ch in range(ch_num):
        ...
        savefig(fig, 'example_res_multichannel_fast_AFD/decomp_comp_level_{:n}_ch_{:n}.jpg'.format(afdcal.level, i_ch))
        fig, _ = afdcal.plot_basis_comp(afdcal.level, i_ch = i_ch)
        savefig(fig, 'example_res_multichannel_fast_AFD/basis_comp_level_{:n}_ch_{:n}.jpg'.format(afdcal.level, i_ch))
        fig, _ = afdcal.plot_re_sig(afdcal.level, i_ch = i_ch)
        savefig(fig, 'example_res_multichannel_fast_AFD/reconstructed_signal_level_{:n}_ch_{:n}.jpg'.format(afdcal.level, i_ch))
        fig, _ = afdcal.plot_energy_rate(afdcal.level, i_ch = i_ch)
        savefig(fig, 'example_res_multichannel_fast_AFD/energy_convergence_rate_level_{:n}_ch_{:n}.jpg'.format(afdcal.level, i_ch))
        fig, _ = afdcal.plot_searchRes(afdcal.level, i_ch = i_ch)
        savefig(fig, 'example_res_multichannel_fast_AFD/searching_result_level_{:n}_ch_{:n}.jpg'.format(afdcal.level, i_ch))
        fig, _ = afdcal.plot_remainder(afdcal.level, i_ch = i_ch)
        savefig(fig, 'example_res_multichannel_fast_AFD/remainder_level_{:n}_ch_{:n}.jpg'.format(afdcal.level, i_ch))
        fig, _ = afdcal.plot_an(afdcal.level, i_ch = i_ch)
        savefig(fig, 'example_res_multichannel_fast_AFD/an_level_{:n}_ch_{:n}.jpg'.format(afdcal.level, i_ch))